...
def login():
    ...
    if user and user["password"] == body.get("password"):
        # Cryptographically random, unguessable session token with no relation
        # to the account id — so a token can never be computed from public,
        # enumerable information (see the predictable-`token-<id>` takeover bug).
