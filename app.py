...
def login():
    ...
    if user and user["password"] == body.get("password"):
        # Use a cryptographically-random, unguessable token. It must not be
        # derivable from the user id (or any public data), so an attacker cannot
        # forge another user's session by guessing their id.
